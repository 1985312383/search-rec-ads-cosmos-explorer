...
这是更激进的一类模式：LLM 不再只是辅助角色，而是直接参与端到端推荐结果的生成。
...
**范式三：LLM作为推荐系统**，这是一类正在快速探索的模式，通过提示工程，让LLM直接参与推荐结果生成或与用户进行对话式推荐。这三种范式展示了LLM从"幕后英雄"到"创意助手"，再到"推荐链路参与者"的演进路径，为构建下一代智能推荐系统提供了可参考的框架。
:::
