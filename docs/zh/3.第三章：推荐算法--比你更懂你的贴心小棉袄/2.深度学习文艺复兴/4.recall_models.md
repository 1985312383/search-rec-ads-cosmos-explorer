...
= \frac{1}{\tau}\sum_{j \in \{i^+\}\cup \mathcal{N}_u}
...
- **含义**：每个正样本 $(u,i^+)$ 所搭配的负样本个数 $K$ 决定了 softmax 近似的精度。
  ...
  $$
  负样本覆盖越充分，越接近全量 softmax 的训练信号；如果采样分布 $q(i)$ 与真实物品分布差异较大，还需要做采样偏差修正，否则会学到带偏的概率。
...
4. [Faiss: A library for efficient similarity search](https://engineering.fb.com/2017/03/29/data-infrastructure/faiss-a-library-for-efficient-similarity-search/) - Facebook关于其ANN引擎Faiss的介绍，了解向量检索背后的工程。
