...
接下来，我们将沿着这一统一框架，依次走近 Wide&Deep、DeepFM、DCN、DIN、DIEN、MMoE 等经典排序模型。
...
设目标物品（候选广告）的 Embedding 为查询向量 $\mathbf{q}\in\mathbb{R}^d$，用户历史行为序列 Embedding 为 $\mathbf{v}_1,\dots,\mathbf{v}_L\in\mathbb{R}^d$。

1. **打分函数**：DIN 中常用一个小型 MLP 作为打分函数 $f_{\text{att}}$，以 $[\mathbf{q},\mathbf{v}_j,\mathbf{q}\odot\mathbf{v}_j,\mathbf{q}-\mathbf{v}_j]$ 等拼接特征为输入：
   $$
   e_j = f_{\text{att}}(\mathbf{q},\mathbf{v}_j) \in \mathbb{R}, \quad j=1,\dots,L.
   $$
2. **注意力权重**：对得分做 softmax 归一化，得到每个历史行为的注意力权重：
   $$
   \alpha_j = \frac{\exp(e_j)}{\sum_{k=1}^L \exp(e_k)}, \quad \sum_j \alpha_j = 1,\; \alpha_j\ge 0.
   $$
3. **兴趣向量聚合**：用权重对历史行为向量做加权求和，得到与当前候选商品相关的兴趣向量：
   $$
   \mathbf{u}_{\text{DIN}} = \sum_{j=1}^L \alpha_j\, \mathbf{v}_j.
   $$

此时，$\mathbf{u}_{\text{DIN}}$ 会与目标物品向量 $\mathbf{q}$、上下文向量等一起拼接，送入后续 DNN 生成最终的 CTR logit $z$。

> 从梯度角度看，越是与候选商品相似、对 loss 贡献大的行为，其对应的 $e_j$ 更大、$\alpha_j$ 更高，反向传播时梯度会集中流经这些行为，从而实现“兴趣聚焦”。

**一个 1 维的小例子**：假设只有两条历史行为，Embedding 为标量 $v_1=1.0, v_2=3.0$，目标物品的打分为 $e_1=0.0, e_2=1.0$，则
$$
\alpha_1 = \frac{e^0}{e^0+e^1} \approx 0.27,\quad
\alpha_2 = \frac{e^1}{e^0+e^1} \approx 0.73,
$$
聚合后的兴趣标量为
$$
...
![DIN模型结构图](/img/ch03/model_DIN.png)
...
`DIEN` (Deep Interest Evolution Network) 可以看作是 DIN 的“进阶版”，它不仅关心**用户当前有哪些兴趣**，还关心这些兴趣 **是如何随时间演化** 出来的。

- **核心出发点**：同样是历史行为序列，早期行为往往代表“老兴趣”，近期行为更代表“新兴趣”；而且不同兴趣之间还会此起彼伏、此强彼弱。
- **关键改进**：
  1. 使用 **GRU/RNN** 对历史行为序列做显式的时间建模，得到每个时刻的“兴趣状态” $\mathbf{h}_t$；
  2. 在兴趣状态序列上再套一层 **类似 DIN 的注意力**，从而找到“对当前候选商品最关键的那些兴趣片段”。
...
1. **GRU 递推**（简化版公式）：
   ...
   \tilde{\mathbf{h}}_t &= \tanh\big(W_h\mathbf{v}_t + U_h(\mathbf{r}_t\odot\mathbf{h}_{t-1}) + \mathbf{b}_h\big),\\
   \mathbf{h}_t &= (1-\mathbf{z}_t)\odot\mathbf{h}_{t-1} + \mathbf{z}_t\odot\tilde{\mathbf{h}}_t.
   \end{aligned}
   $$
   其中 $\mathbf{r}_t$ 为重置门、$\mathbf{z}_t$ 为更新门，控制“保留多少过去记忆 / 引入多少新信息”。

2. **兴趣抽取注意力**：记目标物品向量为 $\mathbf{q}$，在隐藏状态序列上做与 DIN 类似的注意力：
   $$
   e_t = f_{\text{att}}(\mathbf{q},\mathbf{h}_t),\quad
   \alpha_t = \frac{\exp(e_t)}{\sum_{k=1}^T\exp(e_k)},\quad
   \mathbf{u}_{\text{DIEN}} = \sum_{t=1}^T \alpha_t\,\mathbf{h}_t.
   $$

![DIEN模型结构图](/img/ch03/model_DIEN.png)

对比 DIN 直接在 $\mathbf{v}_t$ 上算注意力，DIEN 先用 GRU 把“兴趣随时间的演化”编码到 $\mathbf{h}_t$ 中，再在这些兴趣轨迹上做加权，这在“兴趣切换频繁、序列较长”的场景（如短视频推荐）中更有优势。
...
3. **注意力相关超参：序列长度 & 温度**
   ...
   - 温度系数 $\tau$：在 softmax 中改写为 $\alpha_j = \frac{\exp(e_j/\tau)}{\sum_k \exp(e_k/\tau)}$：
...
5. **样本不平衡：加权 BCE / Focal Loss / 采样策略**
