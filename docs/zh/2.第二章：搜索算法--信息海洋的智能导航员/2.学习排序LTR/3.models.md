...
RankNet是较早将神经网络系统性用于学习排序的经典模型，由微软研究院提出。它的关键洞察是：
- 不需要预测绝对分数，只需要学习相对顺序
- 使用概率模型建模偏好关系
- 通过神经网络学习复杂的非线性关系
...
- 对NDCG的影响差异很大，但RankNet原始损失没有显式区分这种位置影响

### 核心创新：Lambda梯度

LambdaRank的天才之处是**直接定义梯度**，而不是先定义损失函数：

$$\lambda_{ij} = \frac{-\sigma}{1 + e^{\sigma(s_i - s_j)}} \cdot |\Delta_{NDCG}|$$

其中：
- $\sigma$：RankNet 中控制 sigmoid 斜率的超参数；若取 $\sigma=1$，公式可简化为 $\frac{-1}{1+e^{s_i-s_j}}|\Delta_{NDCG}|$
...
3. **指标相关**：让梯度大小与NDCG变化相关，而不是只优化文档对分类误差
...
下面是一个用于理解取舍的示意表，不代表不同数据集上的固定结论：
...
- [XGBoost中的LambdaMART实现](https://xgboost.readthedocs.io/en/stable/tutorials/learning_to_rank.html)
- [LightGBM排序教程](https://lightgbm.readthedocs.io/en/stable/Features.html#ranking)
...
::: tip 🎉 章节小结
从RankNet到LambdaRank再到LambdaMART，展现了LTR技术螺旋上升的发展历程。每个模型都站在前人的肩膀上，解决了特定的问题：RankNet引入了神经网络偏序建模，LambdaRank让梯度与IR指标变化关联，LambdaMART结合了梯度提升树的工程优势。这些经典模型在学术和工业界都影响深远。理解这些模型的设计思想，对于构建高质量的搜索系统很有帮助。
:::
