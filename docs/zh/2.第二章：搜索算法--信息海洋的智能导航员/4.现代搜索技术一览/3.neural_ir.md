...
> **神经检索 (Neural IR)** 是利用深度学习模型（如 BERT, RoBERTa, LLMs）来捕获文本间的复杂语义关系。本节将从底层架构到近年来受到关注的 **RankGPT** 方案进行细化拆解。
...
| :--- | :--- | :--- | :--- |
| **双塔 (Bi-Encoder)** | $score = cos(E_q, E_d)$ | BGE-Base, E5, text-embedding 系列模型 | **召回层** (大规模数据，低延迟响应) |
...
4. **循环**：直到窗口覆盖列表顶端，最终顶部的 Top 1 可视为该滑动窗口策略下得到的较优候选，但不保证等价于全列表的全局最优。
...
- **NDCG@K**：考虑排序顺序的综合得分，是精排评估中常见的核心指标。
...
::: tip 🎉 章节小结
神经检索让搜索引擎从“查词”走向“识意”。在实际工程中，我们通常用**双塔模型召回**控制候选覆盖和延迟，用**知识蒸馏**压缩模型成本，再用 **LLM 排序** 或交叉编码器处理少量高价值候选。这是一场精度、延迟与成本之间的精密权衡。
:::
